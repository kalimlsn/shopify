    ...
    post_hook = ["
                    ...
                    delete from {{ final_table_relation }} where {{ final_table_relation }}._airbyte_unique_key in (
                        ...
                        from (
                                ...
                                from {{ this }}
                                where 1=1 {{ incremental_clause('_airbyte_normalized_at', adapter.quote(this.schema) + '.' + adapter.quote('draft_orders')) }}
                            ) recent_records
                            left join (
                                select _airbyte_unique_key as unique_key, count(_airbyte_unique_key) as active_count
                                from {{ this }}
                                where _airbyte_active_row = 1 {{ incremental_clause('_airbyte_normalized_at', adapter.quote(this.schema) + '.' + adapter.quote('draft_orders')) }}
                    ...
                    {% endif %}
                    ","delete from _airbyte_public.draft_orders_stg where _airbyte_emitted_at != (select max(_airbyte_emitted_at) from _airbyte_public.draft_orders_stg)"],
    tags = [ "top-level" ]
) }}
-- depends_on: ref('draft_orders_stg')
with
{% if is_incremental() %}
new_data as (
    ...
    select
        *
    from {{ ref('draft_orders_stg')  }}
    -- draft_orders from {{ source('public', '_airbyte_raw_draft_orders') }}
    where 1 = 1
...
previous_active_scd_data as (
    ...
    select
        {{ star_intersect(ref('draft_orders_stg'), this, from_alias='inc_data', intersect_alias='this_data') }}
...
input_data as (
    select {{ dbt_utils.star(ref('draft_orders_stg')) }} from new_data
    union all
    select {{ dbt_utils.star(ref('draft_orders_stg')) }} from previous_active_scd_data
),
{% else %}
input_data as (
    select *
